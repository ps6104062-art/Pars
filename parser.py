...
class GiftParser:
    ...
    @staticmethod
    def _stars_price(gift) -> Optional[float]:
        """Return the resale price in Telegram Stars.

        payments.getResaleStarGifts(..., stars_only=True) returns StarsAmount
        values. ``amount`` is the whole-Star part and ``nanos`` is the
        fractional part. TON-only listings are ignored.
        """
        if bool(getattr(gift, "resale_ton_only", False)):
            return None
        ...
        for amount in amounts:
            ...
            return float(raw) + float(nanos) / 1_000_000_000
        return None
            ...
            async with self.sem:
                ...
                result = await self._call(
                    payments.GetResaleStarGiftsRequest(
                        ...
                        sort_by_num=False,
                        for_craft=False,
                        # We need resale listings priced in Telegram Stars.
                        stars_only=True,
    ...
    async def _process_type(self, base_id: int, alert_new: bool):
        try:
            async for result, gifts in self._fetch_pages(base_id):
                for gift in gifts:
                    price = self._stars_price(gift)
                    if price is None:
                        continue
                    if price > config.STAR_PRICE_THRESHOLD:
                        thread_id = config.HIGH_STAR_THREAD_ID
                    elif price < config.STAR_PRICE_THRESHOLD:
                        thread_id = config.LOW_STAR_THREAD_ID
                    else:
                        # Exactly 2000 Stars belongs to neither strict side.
                        continue
                    ...
                    async with self._seller_send_lock:
                        ...
                        log.info(
                            "ALERT %s #%s — %.3f Stars — seller=%s — thread=%s",
                            data["title"], data["number"], price, seller["id"], thread_id,
                        )
        except errors.FloodWaitError as e:
    ...
    async def scan_once(self, alert_new: bool):
        ...
        log.info("Scanning %d gift types for resale gifts priced in Stars around threshold %s...", len(ids), config.STAR_PRICE_THRESHOLD)
        queue = asyncio.Queue()
        for gid in ids:
            await queue.put(gid)
    ...
    async def run(self):
        log.info(
            "Started. Stars threshold=%s; < threshold -> topic %d, > threshold -> topic %d; max profile NFTs=%d",
            config.STAR_PRICE_THRESHOLD, config.LOW_STAR_THREAD_ID, config.HIGH_STAR_THREAD_ID, config.MAX_PROFILE_NFTS,
        )
