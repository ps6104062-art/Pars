...
async def send_alert(client, text: str, buttons):
    """Send an alert. Bot API is preferred because callback buttons require a bot."""
    if config.DRY_RUN:
        print("\n--- DRY RUN ---")
        print(text)
        print("--- END ---\n")
    ...
    if config.BOT_TOKEN and config.BOT_CHAT_ID:
        return await _send_bot_api(text, buttons, config.BOT_CHAT_ID)

    if config.DESTINATION_CHAT:
        # Telethon supports both URL and callback buttons.
...
async def _send_bot_api(text, buttons, chat_id):
